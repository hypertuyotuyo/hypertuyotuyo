(function () {
  ...
  const T = {
    ja: {
      ...
      'gl.ai.c2.note':   'メニューアイコン・テクスチャの幾何学模様・モーション・アニメーション・イラスト・商品画像のロゴ・背景画像・3Dモデル・動画・効果音・ボイス・BGM・音楽・フォント など',
      'gl.ai.c2.note2':  '※ Adobe Firefly のように、学習元がスクレイピングによる無断収集ではなく、権利的に問題のない学習セットであると提供元が明言している生成AIで作成したものに限ります。学習元を明言していない生成AI（学習元が不透明で著作権が判断できないもの）で作成したものは、上記「再現性がなく著作権が不透明なもの」として扱い、使用しません。',
    ...
    en: {
      ...
      'gl.ai.c2.note':   'Menu icons, geometric texture patterns, motion, animations, illustrations, product logos, background images, 3D models, video, sound effects, voices, BGM, music, fonts, etc.',
      'gl.ai.c2.note2':  '※ Limited to works created with generative AI whose provider explicitly states that its training source is a rights-cleared dataset rather than unauthorized scraping — for example, Adobe Firefly. Works created with generative AI that does not disclose its training source (where the source is opaque and copyright cannot be assessed) are treated as "Non-reproducible and copyright-unclear" above and are not used.',
