(function () {
  ...
  const T = {
    ja: {
      ...
      'gl.ai.c2.note2':  '※ Adobe Firefly のように、学習元がスクレイピングによる無断収集ではなく、権利的に問題のない学習セットであると提供元が明言している生成AIに限ります。この場合は、素材用のテクスチャ・マテリアル生成など、生成結果を直接使用することも含みます。学習元を明言していない生成AI（学習元が不透明で著作権が判断できないもの）は、上記「再現性がなく著作権が不透明なもの」として扱い、使用しません。',
    ...
    en: {
      ...
      'gl.ai.c2.note2':  '※ Limited to generative AI whose provider explicitly states that its training source is a rights-cleared dataset rather than unauthorized scraping — for example, Adobe Firefly. In such cases, directly using the generated output — such as generating textures and materials for assets — is also included. Generative AI that does not disclose its training source (where the source is opaque and copyright cannot be assessed) is treated as "Non-reproducible and copyright-unclear" above and is not used.',
