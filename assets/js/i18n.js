(function () {
  ...
  const T = {
    ja: {
      ...
      'gl.h1':             'アセット使用ガイドライン',
      'gl.dates':          '制定日：2024年5月5日　／　最終更新：2026年7月15日',
      ...
      'gl.ai.c2.title':  '著作権的に問題ないと判断できるもの — 使用して作品に組み込むことがあります',
      'gl.ai.c2.note':   '生成結果をそのまま素材として使うのではなく、作品のクオリティを上げるための補助（サポート）として使用するAIに限ります。（アップスケール・ブラッシュアップ・色調補正・ノイズ除去・アニメーション（モーション）支援・フレーム補間・UV展開・トポロジー修正（リトポ）・リギング補助・ウェイト調整・UVパッキング最適化・参考ポーズ生成 など）',
      'gl.ai.c2.note2':  '※ Adobe Firefly のように、学習元がスクレイピングによる無断収集ではなく、権利的に問題のない学習セットであると提供元が明言している生成AIに限ります。この場合は、素材用のテクスチャ・マテリアル生成など、作品の一部の加工に使用することも含みます。学習元を明言していない生成AI（学習元が不透明で著作権が判断できないもの）は、上記「再現性がなく著作権が不透明なもの」として扱い、使用しません。',
      'gl.ai.c3.title':  '再現性がなく著作権が不透明なもの — 使用せず作品に組み込みません',
      'gl.ai.c3.note':   'イラスト・商品画像のロゴ・背景画像・3Dモデル・動画・効果音・ボイス・BGM・音楽・フォント・意訳や物語性のあるテキストなど個人の創造性に依存するもの など',      'gl.ai.c4.title':  '再現性はあるが、クオリティを担保できないもの — 使用せず作品に組み込みません',
    ...
    en: {
      ...
      'gl.dates':          'Established: May 5, 2024 / Last updated: July 15, 2026',
      ...
      'gl.ai.c2.title':  'Elements with no copyright concerns — may be used and incorporated into works',
      'gl.ai.c2.note':   'Limited to AI used as assistance to raise the quality of a work, rather than using its generated output directly as an asset (e.g., upscaling, brush-up, color correction, noise reduction, animation (motion) assistance, frame interpolation, UV unwrapping, topology correction (retopology), rigging assistance, weight adjustment, UV packing optimization, and reference pose generation).',
      'gl.ai.c2.note2':  '※ Limited to generative AI whose provider explicitly states that its training source is a rights-cleared dataset rather than unauthorized scraping — for example, Adobe Firefly. In such cases, using it to work on part of a piece — such as generating textures and materials for assets — is also included. Generative AI that does not disclose its training source (where the source is opaque and copyright cannot be assessed) is treated as "Non-reproducible and copyright-unclear" above and is not used.',
      'gl.ai.c3.title':  'Non-reproducible and copyright-unclear — not used or incorporated into works',
      'gl.ai.c3.note':   'Illustrations, product logos, background images, 3D models, video, sound effects, voices, BGM, music, fonts, paraphrased or narrative text dependent on individual creativity, etc.',      'gl.ai.c4.title':  'Reproducible but quality cannot be guaranteed — not used or incorporated into works',
