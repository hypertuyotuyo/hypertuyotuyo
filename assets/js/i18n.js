(function () {
  ...
  const T = {
    ja: {
      ...
      'gl.ai.c2.note':   '生成結果をそのまま素材として使うのではなく、作品のクオリティを上げるための補助（サポート）として使用するAIに限ります。（アップスケール・ブラッシュアップ・色調補正・ノイズ除去・アニメーション（モーション）支援・UV展開・トポロジー修正（リトポ）・リギング補助 など）',
      'gl.ai.c2.note2':  '※ Adobe Firefly のように、学習元がスクレイピングによる無断収集ではなく、権利的に問題のない学習セットであると提供元が明言している生成AIに限ります。学習元を明言していない生成AI（学習元が不透明で著作権が判断できないもの）は、上記「再現性がなく著作権が不透明なもの」として扱い、使用しません。',
    ...
    en: {
      ...
      'gl.ai.c2.note':   'Limited to AI used as assistance to raise the quality of a work, rather than using its generated output directly as an asset (e.g., upscaling, brush-up, color correction, noise reduction, animation (motion) assistance, UV unwrapping, topology correction (retopology), and rigging assistance).',
      'gl.ai.c2.note2':  '※ Limited to generative AI whose provider explicitly states that its training source is a rights-cleared dataset rather than unauthorized scraping — for example, Adobe Firefly. Generative AI that does not disclose its training source (where the source is opaque and copyright cannot be assessed) is treated as "Non-reproducible and copyright-unclear" above and is not used.',
